...
function displayJson(data) {
    ...
    keys.forEach((key, rowIndex) => {
        ...
        th.style.backgroundColor = rowIndex % 2 === 0 ? "#fffae6" : "#d9f1ff";
        th.style.textAlign = "left";
        row.appendChild(th);
        incidents.forEach((incident, colIndex) => {
            const td = document.createElement("td");
            td.style.border = "1px solid #ddd";
            td.style.padding = "8px";
            td.style.backgroundColor = colIndex % 2 === 0 
                ? (rowIndex % 2 === 0 ? "#f0faff" : "#fffde0")
                : (rowIndex % 2 === 0 ? "#fffde0" : "#f0faff");
            td.style.textAlign = "center";

            if(key === "Actions" && incident.status === "running"){
                const participateButton = document.createElement("button");
                participateButton.textContent = "I want to participate";
                participateButton.style.backgroundColor = "#4CAF50";
                participateButton.style.color = "white";
                participateButton.style.border = "none";
                ...
                participateButton.addEventListener("click", () => {
                    participateInIncident(incident.incident_id, username);
                });
                td.appendChild(participateButton);
            }else if(key === "Share" && incident.status === "running"){
...
function participateInIncident(incidentId, username){
    alert(`Volunteer ${username} has volunteered for incident with ID: ${incidentId}`);
    ...
    .then(responseData => {
        if(responseData.status === 'success'){
            ...
            alert('There was an error notifying the admin.');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        alert('Error notifying the admin.');
    });
}
