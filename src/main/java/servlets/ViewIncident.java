...
public class ViewIncident extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try(PrintWriter out = response.getWriter()){
            ...
            String status = jsonData.optString("status", "all");
            String municipality = jsonData.optString("municipality", "all");
            EditIncidentsTable incidentsTable = new EditIncidentsTable();
            ArrayList<Incident> incidents = incidentsTable.databaseToIncidentsSearch(incidentType, status, municipality);
