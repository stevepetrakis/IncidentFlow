...
public class UpdateUser extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try{
            ...
            eut.updateUserFields(username, requestData);
            User updatedUser = eut.getUserByUsername(username);
            if (updatedUser != null){
                Gson gson = new Gson();
                String userJson = gson.toJson(updatedUser);
                jsonResponse.addProperty("success", true);
                jsonResponse.addProperty("message", "User updated successfully");
                jsonResponse.add("updatedUser", JsonParser.parseString(userJson));
                response.setStatus(HttpServletResponse.SC_OK);
            }else{
                jsonResponse.addProperty("success", false);
                jsonResponse.addProperty("message", "User updated but could not be retrieved");
                response.setStatus(HttpServletResponse.SC_NOT_FOUND);
            }
        }catch(Exception e){
            e.printStackTrace();
            jsonResponse.addProperty("success", false);
            jsonResponse.addProperty("message", "Error while updating the user");
