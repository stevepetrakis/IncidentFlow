...
public class SendMessage extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        EditMessagesTable messageTable = new EditMessagesTable();
        try{
            messageTable.addMessageFromJSON(body);
            response.setStatus(HttpServletResponse.SC_OK);
            response.getWriter().write("{\"success\": \"Message sent successfully.\"}");
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("{\"error\": \"An error occurred while sending the message.\"}");
        }
    }
